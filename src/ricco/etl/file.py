...
def split2x_by_chunksize(
  ...
  if ext == '.csv':
    ...
    df = rdf(filepath)
    to_parts_file(df, dirpath, chunksize=chunksize, to_ext=to_ext, log=log)
...
def split2x(
  ...
  if parts:
    split2x_by_parts(filename, parts=parts, to_ext=to_ext, log=log)
...
def file_to_x(filepath, to_ext,
  ...
  if not overwrite and os.path.exists(to_file_path):
    if log:
      logging.warning(f'文件{filepath}已存在，已跳过')
    return
