...
from .request import request_amap_addr
from .request import request_amap_place
...
def address_json(address: str, city: str, key=None):
  """高德地理编码接口"""
  js = request_amap_addr(address, city, key)
  error_amap(js)
...
def place_json(address: str, city: str, key=None):
  """高德地点检索接口"""
  js = request_amap_place(address, city, key)
  error_amap(js)
  if js['status'] == '1' and int(js['count']) >= 1:
    return js['pois'][0]
